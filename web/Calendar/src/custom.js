    ...
    solar2lunar:function (y,m,d) { //參數區間1900.1.31~2100.12.31
        ...
        for(i=1; i<13 && offset>0; i++) {
            ...
            if(leap>0 && i==(leap+1) && isLeap==false){ 
                --i;
                isLeap = true; temp = calendar.leapDays(year); //計算農歷閏月天數
            }
            else{
                temp = calendar.monthDays(year, i);//計算農歷普通月天數
            }
            //解除閏月
        ...
        var firstNode  = calendar.getTerm(y,(m*2-1));//返回當月「節」為幾日開始
        var secondNode = calendar.getTerm(y,(m*2));//返回當月「節」為幾日開始
...
$(function () {
  ...
  var _tmonths = ['一', '二', '三', '四', '五', '六', '七', '八', '九', '十', '十一', '十二'];
  ...
  function createMonthArr (y, m) {
    ...
    return Array.apply (null, Array (weekCount)).map (function (_, i) {
      return Array.apply (null, Array (7)).map (function (_, j) {
        ...
        /*取得農曆資訊*/
        var lunar = calendar.solar2lunar(y2,m2,d);
        var lunar_m = lunar.IMonthCn;
        var lunar_d = lunar.IDayCn;
        var _lunar = (lunar.lDay == 1) ? lunar.IMonthCn : lunar.IDayCn ;
        var _f = (lunar.lDay == 1) ? true : false ;
        var h = (lunar.nWeek > 0 && lunar.nWeek < 6) ? false : true ;
...
console.log(renderMonth)
    ...
    var ds = monthArr.map (function (w) {
      return $('<div />').addClass ('days').append (
        w.map (function (d) {
          return $('<div />').html("<span class='date'>"+ d.d +"</span><span class='ldate'>"+ d.lunar +"</span>")
                             .addClass (d.l ? null : 'next-prev-month')
                             .addClass (d.t ? 'today' : null)
                             .addClass (d.h ? 'holiday' : null)                 
                             .addClass (d._f ? 'lfirst' : null)
                             .attr ('data-y', d.y)
                             .attr ('data-m', d.m)
                             .attr ('data-d', d.d)
                             .attr ('data-lunar',d.lunar);
        }));
    });

    $month.empty().append (w).append (ds);
    var month = _months[m-1];
    $title.attr('data-y', y).attr('data-m', m).attr('data-em', month);
...
$(document).on('click','.days>div', function (event){
  m = $(this).data('m');
  d = $(this).data('d');
  $('.panel .title').attr ('data-m', m)
                    .attr ('data-d', d);
});